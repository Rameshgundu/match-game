...
class App extends Component {
  ...
  playAgain = () => {
    this.setState({score: 0, count: 60, endGame: false})
  }
  ...
  changeImg = id => {
    ...
    this.setState({randomNum: Math.ceil(Math.random() * imagesList.length)})
  }

  render() {
    const {updatedList, newTabId, randomNum, score, count, endGame} = this.state
    const matchImage = imagesList[randomNum].imageUrl
    const filteredList = updatedList.filter(each => each.category === newTabId)
    const resultCard = endGame ? 'hide-game' : ''
    const scoreCard = endGame ? 'show-card' : 'hide-game'
    return (
                ...
                <ul className="tabs-container">
                  {tabsList.map(eachTab => (
                    <TabsItem
                      ...
                      key={eachTab.tabId}
                      onTab={this.onTab}
                      isActive={eachTab.tabId === newTabId}
                    />
                  ))}
                </ul>
              </div>
